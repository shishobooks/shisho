...
func TestShouldUpdateScalar(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "plugin-specific source cannot overwrite manual identify edit",
			newValue:       "Plugin Title",
			existingValue:  "Edited Identify Name",
			newSource:      models.PluginDataSource("test", "enricher"),
			existingSource: models.DataSourceManual,
			forceRefresh:   false,
			want:           false,
		},
		{
			name:           "embedded metadata cannot overwrite accepted plugin identify name",
			newValue:       "Embedded Title",
			existingValue:  "Accepted Identify Name",
			newSource:      models.DataSourceEPUBMetadata,
			existingSource: models.PluginDataSource("test", "enricher"),
			forceRefresh:   false,
			want:           false,
		},
		{
