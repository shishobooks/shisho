...
- **Cover writes never destroy a working cover.** Write the replacement with `fileutils.WriteFileAtomic` (temp file plus rename, 0644) and only then remove previous covers at other extensions (`removeOtherCoverExtensions` in the scanner, the equivalent loops in `pkg/books` and `pkg/plugins`). Never delete first and write second. When the bytes come from outside the file (an enricher, a download, an upload), the gate is `fileutils.NormalizeImage`'s error, which decodes every pixel; `fileutils.ImageResolution` only reads the header and accepts a truncated body, so it is for resolution comparison, not validation. Embedded covers from the file's own parser are still written best-effort when they do not decode.
