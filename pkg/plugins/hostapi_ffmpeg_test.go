...
func TestFFmpeg_MissingArgs(t *testing.T) {
	...
	_, err := rt.vm.RunString(`shisho.ffmpeg.run()`)
	require.Error(t, err)
...
func TestFFmpeg_RunEcho(t *testing.T) {
	...
	val, err := rt.vm.RunString(`
		var result = shisho.ffmpeg.run(["hello", "world"]);
...
func TestFFmpeg_ProtocolWhitelistPrepended(t *testing.T) {
	...
	val, err := rt.vm.RunString(`
		var result = shisho.ffmpeg.run(["-i", "input.mp4", "output.mp3"]);
		result.stdout;
	`)
	require.NoError(t, err)
...
func TestFFmpeg_StderrCaptured(t *testing.T) {
	...
	val, err := rt.vm.RunString(`
		var result = shisho.ffmpeg.run(["-i", "nonexistent_file_12345.mp4", "-f", "null", "-"]);
		result.stderr;
	`)
	require.NoError(t, err)
...
func TestFFmpeg_ResultFields(t *testing.T) {
	...
	val, err := rt.vm.RunString(`
		var result = shisho.ffmpeg.run(["test"]);
...
func TestFFmpeg_RealFFmpeg(t *testing.T) {
	...
	val, err := rt.vm.RunString(`
		var result = shisho.ffmpeg.run(["-version"]);
