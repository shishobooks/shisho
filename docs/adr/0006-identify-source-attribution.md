...
Earlier Identify clears did leave the plugin source on the absent value. A data migration nulls the source wherever Subtitle, Description, Publisher, URL, or Release Date is absent and the source is a plugin source. It is limited to plugin sources because the Edit form stores a cleared value as absent plus `manual` on purpose, as a protected empty slot, and the scanner never writes an empty value, so a plugin source on an absent value describes nothing and only blocks repopulation. (Deleting a Publisher leaves the same state behind on its Files. The migration heals those rows too, but it can recur, and whether deleting a Publisher should null the source is left open, because a Scan would then re-create the Publisher that was just deleted.) As a second line of defense, clearing an already-absent value through Identify still nulls a leftover source.
