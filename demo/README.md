...
4. Curate metadata, series, genres, tags, and descriptions in the UI. Every Book description must end with a one-line credit and license so attribution is visible without leaving the app. Comic issues and episodes are one Book each inside their Series. To add or replace works, put the files under `library/` (one directory per Book), rescan, and add the work to `CORPUS.md` and `build-corpus.sh`.
