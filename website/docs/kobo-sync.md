...
- **Books with multiple files** — All Kobo-compatible files (EPUB and CBZ) are synced, so each edition gets its own entry on the device. Non-Kobo formats (M4B, PDF) and [supplement files](supplement-files.md) are excluded
